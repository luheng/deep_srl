...
def get_rnn_layer(layer_name):
  ...
  if layer_name in {'highway_no_orth' }:
...
class LSTMLayer(object):
  ...
  def __init__(self, input_dim, hidden_dim, forget_bias = 1.0,
    ...
    self.W = get_variable(_p(prefix, 'W'), [input_dim, 4 * hidden_dim],
                block_orth_normal_initializer([input_dim,], [hidden_dim] * 4))
    self.U = get_variable(_p(prefix, 'U'), [hidden_dim, 4 * hidden_dim],
                block_orth_normal_initializer([hidden_dim,], [hidden_dim] * 4))
    self.b = get_variable(_p(prefix, 'b'), [4 * hidden_dim], all_zero_initializer())
...
class LSTMLayerNoOrth(LSTMLayer):
  ''' Basic LSTM with random normal init instead of ortho normal.
  '''
  def __init__(self, input_dim, hidden_dim, forget_bias = 1.0,
         input_dropout_prob = 0.0, recurrent_dropout_prob = 0.0,
          prefix='lstm'):
...
class HighwayLSTMLayer(LSTMLayer):
  ...
  def __init__(self, input_dim, hidden_dim, forget_bias = 1.0,
    ...
    self.input_dim = input_dim
    self.hidden_dim = hidden_dim
    self.W = get_variable(_p(prefix, 'W'), [input_dim, hidden_dim * 6],
                block_orth_normal_initializer([input_dim,], [hidden_dim] * 6))
    self.U = get_variable(_p(prefix, 'U'), [hidden_dim, hidden_dim * 5],
                block_orth_normal_initializer([hidden_dim,], [hidden_dim] * 5))
    self.b = get_variable(_p(prefix, 'b'), [hidden_dim * 6],
                all_zero_initializer())
    self.params = [self.W, self.U, self.b]
    self.forget_bias = forget_bias
    self.fast_predict = fast_predict
...
class HighwayLSTMLayerNoOrth(HighwayLSTMLayer):
  ''' Highway LSTM without orthonormal initialization.
  '''
  def __init__(self, input_dim, hidden_dim, forget_bias = 1.0,
         input_dropout_prob = 0.0, recurrent_dropout_prob = 0.0,
         prefix='hwlstm'):
...
class SimpleHighwayLSTMLayer(LSTMLayer):
  ...
  def __init__(self, input_dim, hidden_dim, forget_bias = 1.0,
    ...
    self.input_dim = input_dim
    self.hidden_dim = hidden_dim
    self.W = get_variable(_p(prefix, 'W'), [input_dim, hidden_dim * 5],
                block_orth_normal_initializer([input_dim,], [hidden_dim] * 5))
    self.U = get_variable(_p(prefix, 'U'), [hidden_dim, hidden_dim * 5],
                block_orth_normal_initializer([hidden_dim,], [hidden_dim] * 5))
    self.b = get_variable(_p(prefix, 'b'), [hidden_dim * 5],
                all_zero_initializer())
    self.params = [self.W, self.U, self.b]
    self.forget_bias = forget_bias
    self.prefix = prefix
...
class ResidualLSTMLayer(SimpleHighwayLSTMLayer):
  ...
  def __init__(self, input_dim, hidden_dim, forget_bias = 1.0,
    ...
    self.W = get_variable(_p(prefix, 'W'), [input_dim, hidden_dim * 4],
                block_orth_normal_initializer([input_dim,], [hidden_dim] * 4))
    self.U = get_variable(_p(prefix, 'U'), [hidden_dim, hidden_dim * 4],
                block_orth_normal_initializer([hidden_dim,], [hidden_dim] * 4))
    self.b = get_variable(_p(prefix, 'b'), [hidden_dim * 4],
                all_zero_initializer())
    self.params = [self.W, self.U, self.b]
